...
class VulnerabilityScannerApp(tk.Tk):
    ...
    def setup_dashboard_tab(self):
        rel_path = "1000_F_114173456_4XGqS8TjOxiVFUfu47icDC7zSSREQB5L.jpg"  # If the image is in the same directory as the script
        image_path = os.path.join(os.path.dirname(__file__), rel_path)

        original_image = Image.open(image_path)
        self.dashboard_image = ImageTk.PhotoImage(original_image)

        image_label = tk.Label(self.dashboard_tab, image=self.dashboard_image)  # Ensure this is the correct parent widget
