...
class VulnerabilityScannerApp(tk.Tk):
    ...
    def database_setup(self):
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS xss_scan_results
                         (id INTEGER PRIMARY KEY, scanned_url TEXT, 
                         vulnerable TEXT, timestamp TEXT)''')
        self.conn.commit()
    ...
    def insert_xss_scan_result(self, scanned_url, vulnerable):
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            self.cursor.execute('''INSERT INTO xss_scan_results (scanned_url, vulnerable, timestamp)
                                VALUES (?, ?, ?)''', (scanned_url, vulnerable, timestamp))
            self.conn.commit()
        except sqlite3.Error as e:
            print(f"Database error: {e}")
        except Exception as e:
            print(f"Exception in insert_xss_scan_result: {e}")
    ...
    def create_navbar(self):
        ...
        # self.setup_scan_tab("Reflected XSS", "xss", test_reflected_xss_payloads)
        # self.setup_scan_tab("SQL Injection", "sql", self.start_sql_injection)
        # self.setup_scan_tab("Server-Side Template Injection", "ssti", test_server_side_template_injection)
        # self.setup_scan_tab("Open Redirection", "open_redirect", test_open_redirection)
        # self.setup_scan_tab("Clickjacking", "", test_clickjacking) 
    ...
    def scan_xss(self, url, payloads, output):
        ...
        for payload in payloads:
            ...
            try:
                ...
                if payload in response.text:  # Naive check for payload reflection
                    ...
                    vulnerable = "Yes"  # Update vulnerable status if a reflection is found
        ...
        # Insert XSS scan result into database
        self.insert_xss_scan_result(url, vulnerable)
